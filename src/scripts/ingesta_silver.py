    ...
    try:
        ...
        transformacion_capa_silver = query.select(
            [
                ...
                pl.col("metadatos").str.extract(r"^([^,]+)").str.strip_chars().alias("editorial"),
                # Extracción del año si existe de la columna metadatos
                pl.col("metadatos").str.extract(r"(\b\d{4}\b)").cast(pl.Int32, strict=False).fill_null(0).alias("anio"),
                ...
                pl.col("metadatos").str.extract(r"\b(Nuevo|Usado|Como nuevo)\b").str.strip_chars().alias("estado"),
