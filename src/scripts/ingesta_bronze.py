...
async def ingesta_bronze(url: str, headers: dict[str, str], fecha_logica: str, raiz_proyecto: Path) -> str | None:
    """ 
    ...
    async with httpx.AsyncClient(headers=headers, http2=True, transport=transport, timeout=12.0) as client:
        try:
            ...
            for posicion, bloque in enumerate(bloques_libros, start=1):
                # Campos obligarotorios, de haber modificacion de diseño en BuscaLibre, se espera un error para actualizacion del script.
            ...
            df_bronce = pd.DataFrame(records)
            # Creacion de directorios con estándar Hive.
            lake_dir = raiz_proyecto / "lake" / "bronze" / "libros_mas_vendidos"
            bronce_folder: Path = lake_dir / f"year={current_time:%Y}" / f"month={current_time:%m}" / f"day={current_time:%d}"
            ...
            with duckdb.connect(database=':memory:') as con:
                query = f"COPY df_bronce TO '{bronce_file_path.as_posix()}' (FORMAT 'PARQUET', COMPRESSION 'ZSTD')"
                con.execute(query)        
