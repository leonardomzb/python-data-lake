...
def generar_gold(silver_file: str, fecha_logica: str, raiz_proyecto: Path) -> bool:
  try:
    ...
    dim_editorial = (                  
         ...
         .with_columns([
            pl.col("texto_limpio").hash().alias("id_editorial"),
            pl.col("editorial").alias("nombre_editorial")
         ])
    ...
    dim_autor = (
         ...
         .unique(subset=["texto_limpio"])
         .with_columns([
            pl.col("texto_limpio").hash().alias("id_autor"),
            pl.col("autor").alias("nombre_autor")
         ])
    ...
    dim_libro = (
         ...
         .join(dim_autor.with_columns(pl.col("nombre_autor").str.to_lowercase()), left_on="autor", right_on="nombre_autor", how="left")
         .join(dim_editorial.with_columns(pl.col("nombre_editorial").str.to_lowercase()), left_on="editorial", right_on="nombre_editorial", how="left")
    ...
    print(f"¡Proceso de ingesta Gold finalizado con éxito!")
    rutas_gold = {
...
if __name__ == "__main__":
    ...
    SILVER_FILE: str = r'C:\Users\leona\Documents\Trabajo\Trabajo\Python\python-data-lake\lake\silver\libros_mas_vendidos\year=2026\month=07\day=18\data_20260718_134853.parquet'
